...
public class RWMRuleScheduler {
    ...
    ///   - date: The date.
    ///   - stop: The stop.
    public func enumerateDates(startingFrom start: Date, after date: Date? = nil, using block: EnumerationBlock) {
        iterator.enumerateDates(with: rule, startingFrom: start, after: date, calendar: calendar, using: block)
    }
    ...
    public func includes(date: Date, startingFrom start: Date, exact: Bool = false) -> Bool {
        ...
        enumerateDates(startingFrom: start, after: date) { (rdate, stop) in
            if let rdate = rdate {
